...
def data_extraction(pathes) -> None:
    ...
    """ from_city_id, to_city_id = d[0][0]
    from_city, to_city = d[0][1]
    pathes = d[1]
     """
    ...
    # transport set up
    transport_types = {'fly': ('fly', 'flight', 'plane'), 'bus': ('bus', 'nightbus'), 'train': ('train', 'nighttrain', 'cartrain'), 
    ...
    for path_id, path in enumerate(pathes):
        for route in path[8][:-1]:
            ...
            if route[0] in ('fly', 'flight', 'plane'): #transport_types['fly']:
...
def start_threads():        
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        executor.map(data_extraction, gen_jsons())
