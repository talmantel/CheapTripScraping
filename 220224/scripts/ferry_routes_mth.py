...
from settings import ALL_DIRECT_ROUTES_CSV, OUTPUT_JSON_DIR, NO_ID_TRANSPORT_CSV, NOT_FOUND, output_columns_ferry, no_id_transport_set
...
logging.basicConfig(filename=LOGS_FERRY_DIR/'ferry.log', filemode='w', format='%(name)s - %(levelname)s - %(message)s')
...
def data_extraction(city_country, pathes):
    ...
    used_transport_types = ('fly', 'flight', 'plane', 'bus', 'nightbus', 'train', 'nighttrain', 
                            'rideshare', ['busferry', 'ferry', 'carferry', 'trainferry'])

    used_transport_id = {'fly': 1, 'flight': 1, 'plane': 1, 'bus': 2, 'nightbus': 2, 'train': 3, 'nighttrain': 3, 
                        'rideshare': 8, 'ferry': 10, 'carferry': 10, 'busferry': 10, 'trainferry': 10}
    ...
    for path_id, path in enumerate(pathes):
        for route in path[8][:-1]:
            ...
            # for ferries only            
            if route[1] in used_transport_types[-1]: 
    ...
    if found > 0: 
        output_json_ferry(city_country, data)
        data_analyze(data)


def data_analyze(response_data):
       
    temp_df = pd.DataFrame(response_data, columns=output_columns_ferry, index=None)
