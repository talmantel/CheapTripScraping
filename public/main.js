...
async function createArrayCities(id_cities) {
  try {
    ...
    //run on places array
    for (var i = 14517; i < places.length; i++) {
      console.log(i);  
      try{
        const { from, to } = places[i];
        if (!from || !to) return;
        let operation = await search(from, to, id_cities);
        //let operation = await search(id_cities);//  <- Used to check a specific place instead of search(from, to, id_cities);
...
async function search(from, to, id_cities) {
  ...
  //  let from = "Tel Aviv, Israel"; //           <- 
  //  let to = "Tel Aviv Airport (TLV), Israel";//<-  
  try {
    ...
    let data = result[0];  //all options with one means of transport (not integrated)
    var from = result[1];
    var to = result[2];
    var dataOnAllPrices = result[3];  //data on cheap option 
...
function transportId(arr, arrPlus) {
  var arrData = arr;// array information of all data
  var arrDatalines = arrPlus;//data on lines (if have) or empty array
  if(arrDatalines != []){
...
function searchIdCities(from, to, d_sorted, id_cities) {
  ...
  let checkNamesFrom = [];
  let checkNamesTo = [];
  for (let i = 0; i < id_cities.length; i++) {
    let id = id_cities[i].city;
