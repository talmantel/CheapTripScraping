...
def get_inner_json(pth, rt, route_dic):
    try:
        if route_dic[pth][8][rt][0] in ['walk', 'car', 'hotel', 'tram']:
            ...
            return {
                    ...
                    "plane_change":route_dic[pth][8][rt][12][0],
                    "time_travel":route_dic[pth][8][rt][5],
                    "time_2?":     route_dic[pth][8][rt][6],
                    "time_3?":     route_dic[pth][8][rt][7],
                    }
        else:
            return {
...
def main():
    # f = open('358-62-Vaxjo-Vancouver.json', 'r')
    ...
    f.close()
    for i, path in enumerate(route_dic):
        for j, _ in enumerate(path[8]):
            q = get_inner_json(i, j, route_dic)
            if type(q) == dict: print(q, '\n')
